...
const Totem = ({ isRotating, setIsRotating, ...props }) => {
  const totemref = useRef();
  const { gl, viewport } = useThree();
  const { nodes, materials, animations } = useGLTF(totemscene)
  const { actions } = useAnimations(animations, totemref)
  ...
  const handlePointerMove = (e) => {
    ...
    totemref.current.rotation.y += delta * 0.01 * Math.PI;
    lastX.current = clientX;
  ...
  useFrame(() => {
    if(!isRotating) {
      ...
      const rotation = totemref.current.rotation.y;
    ...
    <a.group ref={totemref} {...props} >
       <group name="Scene">
        <group name="ThreeJs02glb">
          ...
          <group name="Totem_Logo_Websiteglb">
            <mesh
              name="Totem_logo001"
              castShadow
              receiveShadow
